              ...
              return (<div key={unit.id}>
                ...
                {!uh && unit.lessons?.map((lesson: any) => {
                  ...
                  return (<div key={lesson.id}>
                    <button onClick={() => toggleL(lesson.id)} className="w-full text-left pl-5 pr-2 py-1.5 text-sm text-foreground hover:bg-accent/50 rounded-md flex items-center gap-1.5" title={lesson.title}>
                      <span className="text-muted-foreground text-[10px]">{lh ? "▸" : "▾"}</span>
                      <span className="truncate font-medium">{lesson.title}</span>
                    </button>
                    {!lh && secs.map((sec: any) => (
                      <button key={sec.id} onClick={() => loadBank(sec.id, sec.title)} title={sec.title}
                        className={`w-full text-left pl-10 pr-2 py-1 rounded-md text-sm transition-colors truncate ${selSection === sec.id ? "bg-primary/10 text-primary font-medium" : "text-muted-foreground hover:text-foreground hover:bg-accent/50"}`}>
                        {sec.title}
                      </button>
                    ))}
                  </div>);
