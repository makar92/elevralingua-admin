...
export default function StudentInvitations() {
  const { data: incoming = [], isLoading: l1 } = usePolling<any[]>("/api/invitations?direction=received&type=TEACHER_INVITES", { fallback: [] });
  const { data: myRequests = [], isLoading: l2 } = usePolling<any[]>("/api/invitations?direction=sent&type=STUDENT_REQUESTS", { fallback: [] });
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="flex gap-2 mb-6">
        <button onClick={() => setTab("incoming")}
          ...
          From Teachers {incoming.filter(i => i.status === "PENDING").length > 0 && (
      ...
      {items.length === 0 ? (
        <p className="text-muted-foreground text-center py-12">
          {tab === "incoming" ? "No invitations from teachers" : "No requests sent"}
        </p>
      ) : (
