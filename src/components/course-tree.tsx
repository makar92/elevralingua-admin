...
//   - У курса справа кнопка-карандаш для редактирования параметров (onEditCourse).
//   - Клик по названию Unit/Lesson — только разворачивает/сворачивает поддерево (без select).
//   - Клик по "📕 Textbook" / "📓 Workbook" — также разворачивает/сворачивает.
//   - Клик по названию секции (textbook/workbook) — выбирает секцию (открывает редактор справа).
// ===========================================
...
export function CourseTree({ course, selectedId, onSelect, onAddLesson, onAddTextbookSection, onAddWorkbookSection, onRename, onMove, onDelete, onEditCourse }: {
    ...
    <div className="space-y-1 text-sm">
      <div className="flex items-center group">
        <button onClick={() => onSelect({ type: "course", id: course.id, data: course })}
          className={`flex-1 text-left px-3 py-2 rounded-md font-semibold transition-colors truncate ${
        ...
        <button onClick={(e) => { e.stopPropagation(); onEditCourse(); }}
          className="w-6 h-6 flex items-center justify-center rounded text-muted-foreground hover:text-foreground hover:bg-accent opacity-0 group-hover:opacity-100 ml-1 flex-shrink-0"
          title="Edit Course">
      ...
      {course.units.map((unit, unitIdx) => (
        <div key={unit.id} className="ml-2">
          <div className="flex items-center group">
            ...
            <button onClick={() => setExpandedUnits(toggle(expandedUnits, unit.id))}
              className="flex-1 text-left px-2 py-1.5 rounded-md transition-colors truncate text-foreground hover:bg-accent"
          ...
          {expandedUnits.has(unit.id) && unit.lessons.map((lesson, lessonIdx) => (
            <div key={lesson.id} className="ml-5">
              <div className="flex items-center group">
                ...
                <button onClick={() => setExpandedLessons(toggle(expandedLessons, lesson.id))}
                  className="flex-1 text-left px-2 py-1 rounded-md transition-colors truncate text-foreground hover:bg-accent"
              ...
              {expandedLessons.has(lesson.id) && (
                <div className="ml-5">
                  ...
                  <div className="flex items-center group mt-0.5">
                    ...
                    <button onClick={() => setExpandedTextbook(toggle(expandedTextbook, lesson.id))}
                      className="flex-1 text-left px-2 py-1 text-muted-foreground text-xs font-semibold uppercase tracking-wider hover:bg-accent hover:text-foreground rounded-md transition-colors">📕 Textbook</button>
                    <Button variant="ghost" size="sm"
                  ...
                  <div className="flex items-center group mt-0.5">
                    ...
                    <button onClick={() => setExpandedWorkbook(toggle(expandedWorkbook, lesson.id))}
                      className="flex-1 text-left px-2 py-1 text-muted-foreground text-xs font-semibold uppercase tracking-wider hover:bg-accent hover:text-foreground rounded-md transition-colors">📓 Workbook</button>
                    <Button variant="ghost" size="sm"
                      className="h-5 w-5 p-0 opacity-0 group-hover:opacity-100 text-muted-foreground"
